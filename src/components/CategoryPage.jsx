...
// ─────────────────────────────────────────────────────────────────────
// BUGS FIXED in the inline EnquiryModal (lines ~37-120 of doc-7):
//
//  BUG 1 — Promise.all meant if EITHER Supabase OR GAS threw, the
//    entire submit failed with a blank catch block.
//    FIX: Supabase runs first (required), GAS runs after (non-blocking).
//
//  BUG 2 — Email had no `required` attribute. Users could submit
//    without an email → GAS rejected silently.
//    FIX: email field now has required + type="email".
//
//  BUG 3 — No error state in the inline modal — failures were invisible.
//    FIX: added err state + error display inside the form.
//
//  BUG 4 — form.name sent as fullName to GAS correctly, but
//    form.email wasn't validated before calling submitEnquiry, so
//    GAS could get an empty email string.
//    FIX: client-side email guard added before submit.
//
// CategoryPage structure, styles, PackageCard, testimonials — ALL UNCHANGED.
// ─────────────────────────────────────────────────────────────────────
...
import {
  Plane, Hotel, Utensils, Activity, Car,
  Quote, Ship, Star, X, ChevronRight, MapPin, Clock, ArrowRight, AlertCircle
} from 'lucide-react';
import { supabase }      from '../supabaseClient';
import { submitEnquiry } from '../services/enquiryService';
...
const testimonials = [
  { init: 'JM', name: 'Jestin Mathew',  text: 'We would like to appreciate your good office for arranging travel trip to Maldives for our Director, Mr Walfred Tagor...' },
  { init: 'RB', name: 'Ramesh Babu',    text: 'Dear Sir, We had arranged a vehicle for Mr. Swithun Manoharan - Executive Vice President. The service was impeccable.' },
  { init: 'SK', name: 'Santosh Krinsky',text: 'I wanted to take the opportunity to thank you for your efforts in making our trip so comfortable and memorable.' },
];

/* ─────────────────────────────────────────────────────────────
   ENQUIRY MODAL — fixed version
   (replaces the inline one that was broken in doc-7)
──────────────────────────────────────────────────────────── */
const EnquiryModal = ({ pkg, onClose }) => {
  ...
  const [sending, setSending] = useState(false);
  const [done,    setDone]    = useState(false);
  const [err,     setErr]     = useState('');   // ← was missing in original
  ...
  const handleSubmit = async (e) => {
    ...
    // ── Client-side guard ──────────────────────────────────────────
    if (!form.email.trim()) {
      setErr('Email address is required so we can send you a confirmation.');
      return;
    }
    ...
    try {
      ...
      if (sbError) throw new Error('Could not save your enquiry. Please try again.');

      // ── STEP 2: Call GAS for emails (non-blocking) ────────────
      // We fire-and-forget so a GAS timeout never blocks the user.
      submitEnquiry({
        fullName:     form.name,
        phoneNumber:  form.phone,
        email:        form.email,
        fromDate:     form.travel_date,
        travelers:    String(form.pax),
        requirements: form.message,
        serviceType:  'holiday',
        packageName:  pkg?.name || '',
      }).catch(gasErr => {
        console.warn('GAS email (non-blocking):', gasErr.message);
      });

      setDone(true);

    } catch (catchErr) {
      setErr(catchErr.message || 'Something went wrong. Please try again.');
    } finally {
      setSending(false);
    }
  };
      ...
      >
        ...
        {done ? (
          <div className="p-8 text-center">
            ...
            <h3 className="font-black text-slate-900 text-xl mb-2">Enquiry Sent!</h3>
            <p className="text-slate-500 text-sm mb-6">Our team will contact you within 24 hours.</p>
            <button onClick={onClose} className="bg-blue-600 text-white px-8 py-3 font-black text-xs uppercase tracking-widest hover:bg-blue-700 transition-colors">
              Close
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
            ...
            {/* ── FIX: now required with helper text ── */}
            <div>
              <label className="field-label">
                Email * <span style={{ fontSize: 9, fontWeight: 400, textTransform: 'none', color: '#94a3b8' }}>(for confirmation email)</span>
              </label>
              <input
                type="email"
                required                   
            ...
            {/* ── FIX: error now surfaces to user ── */}
            {err && (
              <div className="flex items-start gap-2 bg-red-50 border border-red-100 rounded px-3 py-2">
  ...
  return (
    ...
    >
      ...
      >
        ...
        {daysLabel && (
          <div className="absolute top-2 right-2 bg-blue-600 text-white text-[8px] font-black uppercase px-2 py-1 tracking-widest z-10">{daysLabel}</div>
        )}
        {pkg.price && (
          <div className="absolute bottom-0 right-0 bg-slate-900/90 text-white px-3 py-1.5 backdrop-blur-md">
            <span className="text-[7px] block leading-none opacity-60 uppercase font-bold text-left">From</span>
      ...
      <div className="flex items-center justify-center gap-1.5 mb-3 text-blue-600">
        <MapPin size={10} />
        <span className="text-[9px] font-bold uppercase tracking-wider line-clamp-1">{pkg.route_covering || "Premium Experience"}</span>
      </div>
      <p className="text-slate-500 text-[11px] leading-relaxed font-medium mb-6 px-4 line-clamp-2 flex-1">
        {pkg.description || "Discover the hidden gems of this destination with our curated tour."}
      </p>
      <div className="mt-auto flex flex-col gap-2 w-full px-8 pb-4">
        <button onClick={() => onEnquire(pkg)} className="w-full bg-slate-900 text-white py-2 text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all duration-300">
          Enquire Now
        </button>
        <Link to={`/package/${pkg.id}`} className="w-full border border-slate-800 text-slate-800 py-1.5 text-[9px] font-black uppercase tracking-widest hover:bg-slate-800 hover:text-white transition-all duration-300 text-center">
          Read More
        </Link>
      </div>
    </motion.div>
...
/* ─── MAIN COMPONENT (UNCHANGED except enquiry state) ────────── */
const CategoryPage = ({ type = 'india' }) => {
  const { slug }  = useParams();
  const navigate  = useNavigate();
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!catData) { navigate(type === 'india' ? '/tours/india' : '/tours/international'); return; }
      setCat(catData);
  ...
  return (
    <div className="cat-page-root">
      ...
      <div className="cat-hero">
        ...
        <div className="cat-hero-orb cat-hero-orb-2" />
        <div className="cat-page-inner cat-hero-inner">
          <motion.div className="cat-breadcrumb" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <Link to="/" className="cat-bc-link">Home</Link>
            <ChevronRight size={11} />
            <Link to={backLink} className="cat-bc-link">{backLabel}</Link>
            <ChevronRight size={11} />
          ...
          <motion.button onClick={() => window.history.length > 1 ? navigate(-1) : navigate(backLink)} className="cat-back-btn" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
            <span className="cat-back-icon">
          ...
          <div className="cat-hero-content">
            <motion.div className="cat-hero-text" initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.15 }}>
              <div className="cat-eyebrow"><span className="cat-eyebrow-line" /><span className="cat-eyebrow-txt">{cat.tagline || cat.name}</span></div>
              <h1 className="cat-heading">
                <span className="cat-heading-line1">{line1}</span>
                {line2 && <span className="cat-heading-line2">{line2}</span>}
              </h1>
              <p className="cat-hero-desc">{cat.description || `Explore the breathtaking beauty of ${cat.name} with our expertly curated packages, crafted for unforgettable memories.`}</p>
              <div className="cat-stats-strip">
                <div className="cat-stat"><span className="cat-stat-num">{packages.length}+</span><span className="cat-stat-label">Packages</span></div>
                <div className="cat-stat-divider" />
                <div className="cat-stat"><span className="cat-stat-num">100%</span><span className="cat-stat-label">Customisable</span></div>
                <div className="cat-stat-divider" />
                <div className="cat-stat"><span className="cat-stat-num">24/7</span><span className="cat-stat-label">Support</span></div>
              </div>
              <div className="cat-inclusions">
                {inclusions.map((inc, i) => (
                  <motion.div key={i} className="cat-inclusion-pill" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.07 }}>
                    {inc.icon}<span>{inc.text}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
            <motion.div className="cat-hero-img-card" initial={{ opacity: 0, scale: 0.88, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.2, ease: [0.25, 0.46, 0.45, 0.94] }}>
              {cat.image_url
                ? <img src={cat.image_url} alt={cat.name} className="cat-hero-img" />
                : <div className="cat-hero-img cat-hero-img-ph"><Ship size={80} className="text-blue-200" /></div>
              }
              <div className="cat-hero-img-overlay" />
              <div className="cat-hero-img-badge"><Star size={12} className="fill-amber-400 text-amber-400" /><span>Top Destination</span></div>
            </motion.div>
          </div>
        </div>
      </div>
      ...
      <div className="cat-packages-section">
        <div className="cat-page-inner">
          <motion.div className="cat-section-header" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <div className="cat-section-label-row">
              <span className="cat-section-line" />
              <span className="cat-section-label-txt">{cat.name} Packages</span>
              <span className="cat-section-count">{packages.length} packages</span>
            </div>
            <h2 className="cat-section-heading">Choose Your <span className="cat-section-heading-blue">Perfect Getaway</span></h2>
          </motion.div>
          {packages.length === 0
            ? <div className="cat-empty-state"><div className="cat-empty-icon"><Ship size={40} /></div><p className="cat-empty-title">Packages coming soon</p><p className="cat-empty-sub"><Link to="/contact" className="cat-empty-link">Contact us</Link> for custom itineraries</p></div>
            : <div className="cat-grid">{packages.map((pkg, idx) => <PackageCard key={pkg.id} pkg={pkg} catImage={cat.image_url} index={idx} onEnquire={setEnquiry} />)}</div>
          }
        </div>
      </div>
          ...
          <div className="bg-white p-8 shadow-xl border border-blue-50 relative">
            ...
            <AnimatePresence mode="wait">
              <motion.div key={testIdx} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.3 }}>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-blue-600 rounded-none flex items-center justify-center text-white text-[10px] font-bold">{testimonials[testIdx].init}</div>
                  <p className="font-black text-slate-900 text-[11px] uppercase tracking-wider">{testimonials[testIdx].name}</p>
                </div>
              </motion.div>
            </AnimatePresence>
            <div className="flex gap-1.5 mt-5">
              {testimonials.map((_, i) => <button key={i} onClick={() => setTestIdx(i)} className={`h-1.5 rounded-full transition-all duration-300 ${i === testIdx ? 'w-5 bg-blue-600' : 'w-1.5 bg-gray-200'}`} />)}
            </div>
          </div>
          <div className="lg:col-span-2 bg-gradient-to-br from-blue-900 to-slate-900 p-12 relative flex flex-col justify-center rounded-none">
            <h2 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter mb-4">Explore Your <br /><span className="text-cyan-400">Dream Destination!</span></h2>
            <p className="text-blue-200 font-bold italic mb-8">You'll never forget the experience we are ready to present Joy!</p>
            <div className="flex gap-4">
              <Link to="/enquery" className="bg-white text-slate-900 px-12 py-4 font-black uppercase tracking-widest text-xs hover:bg-cyan-400 transition-all inline-block">Click Here</Link>
            </div>
          </div>
        </div>
      </div>
